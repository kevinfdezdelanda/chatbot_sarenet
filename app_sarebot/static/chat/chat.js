	...
	fetch(`cargar_chats/?chat_id=${chatId}`)
		...
		.then((data) => {
			...
			data.forEach((registro) => {
				...
				preguntasMap.set(registro.pregunta, registro.respuesta);
			});

			// Luego recorremos el Map para añadir las preguntas y respuestas al chatbox
			preguntasMap.forEach((respuesta, pregunta) => {
				addMessageToChatbox(pregunta, 'You');
				addMessageToChatboxChatHistory(md.render(respuesta));
			});
