...
function preserveEmptyLines(md) {
    md.core.ruler.push('preserve_empty_lines', (state) => {
        const tokens = state.tokens;
        let i = 0;
        while (i < tokens.length) {
            if (tokens[i].type === 'inline' && tokens[i].content === '') {
                const emptyToken = new state.Token('html_block', '\n', 0);
                emptyToken.content = '<p>&nbsp;</p>';
                tokens.splice(i, 0, emptyToken);
                i++;
            }
            i++;
        }
...
function crear_conversor_markdown() {
    ...
    // Agrega la extensión personalizada para preservar líneas vacías
    md.use(preserveEmptyLines);
